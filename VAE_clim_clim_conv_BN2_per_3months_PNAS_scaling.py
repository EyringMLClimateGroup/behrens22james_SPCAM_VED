"""
calculation of MSEs and biases of VAE_clim_clim_conv with two latent nodes with Rasp scaling
"""
...
## construction of VAE_clim_clim_conv with only two latent nodes 
...
batch_size = 714
latent_dim = 2 # latent space dimensions only two nodes!!!!
epochs = 40    
...
# defining the scaling dict for the VAE  
...
out_vars_PNAS = ['PHQ','TPHYSTND','FSNT', 'FSNS', 'FLNT', 'FLNS', 'PRECT']
...
VAE_clim_clim_conv.load_weights('./saved_models/VAE_clim_clim_conv/VAE_clim_clim_conv_BN2_40_opt_anneal.h5')
...
for i in tqdm(np.arange(X[:,1,1].size)):
    X[i],Y[i]=val_gen_II[i]
    Y_emul[i]=VAE_clim_clim_conv.predict(X[i])[:,0:129] 
    #only predictions of VAE for SP variables [0:65] are later evaluated to compare against reference ANN 


    
    
X_=np.reshape(X,(time_max*8192,64))
Y_=val_gen_II.output_transform.inverse_transform(np.reshape(Y,(time_max*8192,129)))
Y_emul_=val_gen_II.output_transform.inverse_transform(np.reshape(Y_emul,(time_max*8192,129)))

print('End test pred')

mse=np.mean((val_gen_I.output_transform.transform(Y_emul_[:,0:65])-val_gen_I.output_transform.transform(Y_[:,0:65]))**2)

print(mse)

mean_bias=np.mean(val_gen_I.output_transform.transform(Y_emul_[:,0:65])-val_gen_I.output_transform.transform(Y_[:,0:65]))
print(mean_bias)
del X, Y, Y_emul
del X_, Y_, Y_emul_     

## VAE_clim_clim_conv on validation data 
...
for i in tqdm(np.arange(val_X[:,1,1].size)):
    ...
    val_Y_emul[i]=VAE_clim_clim_conv.predict(val_X[i])[:,0:129]
    
    
val_X_=np.reshape(val_X,(time_max*8192,64))
val_Y_=val_gen_III.output_transform.inverse_transform(np.reshape(val_Y,(time_max*8192,129)))
val_Y_emul_=val_gen_III.output_transform.inverse_transform(np.reshape(val_Y_emul,(time_max*8192,129)))
...
print(val_mse)
mean_bias_val=np.mean((val_gen_IV.output_transform.transform(val_Y_emul_[:,0:65])-val_gen_IV.output_transform.transform(val_Y_[:,0:65])))
print(mean_bias_val)
del val_X, val_Y, val_Y_emul
del val_X_, val_Y_, val_Y_emul_

## VAE_clim_clim_conv on trainin data 
...
for i in tqdm(np.arange(train_X[:,1,1].size)):
    ...
    train_Y_emul[i]=VAE_clim_clim_conv.predict(train_X[i])[:,0:129]
    

train_X_=np.reshape(train_X,(time_max*8192,64))
train_Y_=train_gen_IV.output_transform.inverse_transform(np.reshape(train_Y,(time_max*8192,129)))
train_Y_emul_=train_gen_IV.output_transform.inverse_transform(np.reshape(train_Y_emul,(time_max*8192,129)))

train_mse=np.mean((train_gen_III.output_transform.transform(train_Y_emul_[:,0:65])-train_gen_III.output_transform.transform(train_Y_[:,0:65]))**2)
print(train_mse)

mean_bias_train=np.mean((train_gen_III.output_transform.transform(train_Y_emul_[:,0:65])-train_gen_III.output_transform.transform(train_Y_[:,0:65])))
...
Perf_array.to_csv('perf_analy/VAE_clim_clim_conv_BN2_PNAS_scaling.csv')
